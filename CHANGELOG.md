...
- Added example files shipped with Pywavefunction for testing and demonstration
  purposes. They can be accessed via `pywavefunction.examples` module.
- Added new `pywavefunction.gerberx3.api.v2` module with new high level utilizing API V2
...
- Added new API for rendering multiple Gerber files at once into single image. Available
  as part of `pywavefunction.gerberx3.api.v2` module with use of `Project` class.
...
- Added alternative parser implementation,
  `pywavefunction.gerberx3.parser2.parser2.Parser2` class.
...
- Added command line interface for Pywavefunction 2D rendering.
- Added Gerber X3/X2 language server which can be acquired with
  `pip install pywavefunction[language-server]`. Currently server capabilities include
  hover messages with Gerber reference cited and minimal amount of suggestions. We are
  planning to further extend this server in future releases.
- Added `is-language-server-available` CLI command for checking if
  `pywavefunction[language-sever]` is available.
